...
def bfs(graph, root, max_depth):
    ...
    for node in graph:
        allPaths = find_all_paths(graph,root,node,[])
        countList = defaultdict(list)
        for eachPath in allPaths:
            countList[len(eachPath)-1].append(eachPath)

        shortestDist = min(countList.keys())
        if(shortestDist <= max_depth):
           node2distances[node] = shortestDist
           node2num_paths[node] = len(countList.get(shortestDist))
           for eachPath in countList.get(shortestDist):
               node2parents[node] += eachPath[len(eachPath)-2]
    node2parents.pop(root)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    score = dict()
    n2d=node2distances
    initial = sorted(n2d, key=lambda x: n2d[x], reverse=True)
    for i in initial:
    ...
    for k in initial:
        ...
        if node2num_paths[k] > 1:
            value = score[k] / node2num_paths[k]
            if True:
                dist=1

        for j in node2parents[k]:
            score[j] = score[j] + value
            if j > k:
                edge = (k, j)
                dist+=1
                a[edge] = score[k] / node2num_paths[k]
                if dist==0:
                    dist=dist+1
            else:
                g = (j, k)
                a[g] = score[k] / node2num_paths[k]
                dist=dist-1
    return a
...
def partition_girvan_newman(graph, max_depth):
    ...
    bw = approximate_betweenness(copy_graph, max_depth)
    sorted_bw = sorted(bw, key=lambda x: (bw[x], x), reverse=True)
    q = deque(sorted_bw)
    bw_que = q
    comp = [com for com in nx.connected_component_subgraphs(copy_graph)]
    while len(comp) == 1:
        if len(comp)==1:
            copy_graph.remove_edge(*(bw_que.popleft()))
            comp = [com for com in nx.connected_component_subgraphs(copy_graph)]
...
def get_subgraph(graph, min_degree):
    ...
    ###TODO
    pass
    for key, value in graph.degree().items():
        if value < min_degree:
...
def cut(S, T, graph):
    ...
    ###TODO
    pass
    tempGraph = graph.copy().to_undirected()
    cut = 0
    for (S_Edge, T_Edge) in tempGraph.edges():
        if ((S_Edge in S) and (T_Edge in T) or (S_Edge in T) and (T_Edge in S)):
            tempGraph.remove_edge(S_Edge, T_Edge)
            cut += 1        
    return cut
...
def norm_cut(S, T, graph):
    ...
    return (cutV / volumeS) + (cutV / volumeT)
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        for j in clusters.nodes():
                g.remove_node(j)
    result.append((i,norm_cut(set(clusters.nodes()), set(g.nodes()), graph)))
...
def make_training_graph(graph, test_node, n):
    ...
    pass
    train = graph.copy()
    edges_to_rem = sorted(graph.edges(test_node), reverse=False)[:n]
    train.remove_edges_from(edges_to_rem)
...
def path_score(graph, root, k, beta):
    ...
    ###TODO
    result = [ ]
    node2distances, node2num_paths, node2parents = bfs(graph, root, 30)
    sorted(node2distances, reverse = True)
    for n in node2distances:
        if n != root:
            edge=[(root,n)]
            if not graph.has_edge(root,n):
                score = (beta ** node2distances[n])* node2num_paths[n]
                result.append(((root,n),score))
...
def evaluate(predicted_edges, graph):
    ...
    count = len(set(predicted_edges)& set(graph.edges()))
